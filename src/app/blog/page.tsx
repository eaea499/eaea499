...
export default function BlogPage() {
  return <><SiteHeader /><main className="inner-page"><div className="site-wrap page-heading blog-heading"><p className="section-kicker">Notes</p><h1>把踩过的坑和<br />想明白的事写下来。</h1><p>记录项目复盘、开发过程和学习理解。不追求高产，只写确实做过、真正想清楚的内容。</p></div><section className="site-wrap draft-list" aria-label="文章列表">{notes.map((note, index) => {
    const content = <><div className="draft-index">0{index + 1}</div><div><p className="detail-kind">{note.category} / {note.status ?? "即将发布"}</p><h2>{note.title}</h2><p>{note.description}</p></div><BookOpen size={24} strokeWidth={1.5} /></>;
    return note.href ? <Link className="draft-link" href={note.href} key={note.title}>{content}</Link> : <article key={note.title}>{content}</article>;
