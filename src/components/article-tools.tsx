...
export function ArticleTools() {
  ...
  return <>
    ...
    <aside className="article-tools" aria-label="文章工具">
      <button onClick={copyLink} type="button" title="复制文章链接">{copied ? <Check size={17} /> : <Copy size={17} />}<span>{copied ? "已复制" : "复制链接"}</span></button>
      <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} type="button" title="返回顶部"><ArrowUp size={17} /><span>顶部</span></button>
    </aside>
  </>;
}
